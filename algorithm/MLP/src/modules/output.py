...
def save_scaling(scaling_params):
    ...
    np.save(glob.out_dir + "/scaling", scaling_params)
    if glob.wandb_logging:
        scale_artifact = wandb.Artifact("scaling-parameters", type="parameters")
        scale_artifact.add_file(glob.out_dir + "/scaling.npy")
        wandb.log_artifact(scale_artifact, aliases=["latest", glob.run_name])
...
def save_model(model, optimizer, example, best_model=None):
    ...
    best_model_path = glob.out_dir + "/model.pt"
    checkpoint_path = glob.out_dir + "/ckp_model.pt"
    save_ckp(checkpoint, False, checkpoint_path, best_model_path)

    # save model for inference
    scripted_model = torch.jit.trace(model.eval().to("cpu"), example)
    scripted_model.save(glob.out_dir + "/scripted_model.pt")
    if best_model is not None:
        best_scripted_model = torch.jit.trace(best_model.eval().to("cpu"), example)
        best_scripted_model.save(glob.out_dir + "/scripted_model_best.pt")

    # Log the model to wandb
    if glob.wandb_logging:
        model_artifact = wandb.Artifact("model", type="model")
        model_artifact.add_file(checkpoint_path)
        model_artifact.add_file(glob.out_dir + "/scripted_model.pt")
        if best_model is not None:
            model_artifact.add_file(glob.out_dir + "/scripted_model_best.pt")
        wandb.log_artifact(model_artifact, aliases=["latest", glob.run_name])
...
def write_hystory(history):
    ...
    np.save(glob.out_dir + "/loss", history)
...
def write_datasets(indices, train_num, val_num):
    ...
    df.to_excel(glob.out_dir + "/indices.xlsx", index=False)
    ...
    if glob.wandb_logging:
        ...
        wandb.log_artifact(table_artifact, aliases=["latest", glob.run_name])
