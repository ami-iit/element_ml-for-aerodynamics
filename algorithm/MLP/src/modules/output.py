...
def save_model(model, optimizer, example, best_model=None):
    ...
    if best_model is not None:
        ...
        best_scripted_model.save(Const.out_dir + "/scripted_model_best.pt")
        torch.onnx.export(
            best_model.eval().to("cpu"),
            example,
            Const.out_dir + "/model_best.onnx",
            export_params=True,
            opset_version=10,
            do_constant_folding=True,
            input_names=["input"],
    ...
    if Const.wandb_logging:
        ...
        model_artifact.add_file(Const.out_dir + "/scripted_model.pt")
        model_artifact.add_file(Const.out_dir + "/model.onnx")
        if best_model is not None:
            model_artifact.add_file(Const.out_dir + "/scripted_model_best.pt")
            model_artifact.add_file(Const.out_dir + "/model_best.onnx")
        wandb.log_artifact(model_artifact, aliases=["latest", Const.run_name])


def write_hystory(history):
    print("Saving loss history as loss.npy")
    np.save(Const.out_dir + "/loss", history)
...
def write_datasets(indices, train_num, val_num):
    ...
    df.to_excel(Const.out_dir + "/indices.xlsx", index=False)
    ...
    if Const.wandb_logging:
        ...
        wandb.log_artifact(table_artifact, aliases=["latest", Const.run_name])
