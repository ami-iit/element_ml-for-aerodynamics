...
class Robot:
    ...
    def compute_all_world_H_link(self):
        ...
        for surface_id, surface_name in enumerate(self.surface_list):
            world_H_link = self.compute_world_H_link(frame_name=self.surface_frames[surface_id])
            frame_rotation_matrix = R.from_euler('z', self.rotation_angles[surface_id], degrees=True).as_matrix()
        ...
        for link_id in range(model.getNrOfLinks()):
            ...
            for link_visual in visuals[link_id]:
                if link_visual.isExternalMesh():
                    ...
                    link_H_mesh = link_visual.getLink_H_geometry().asHomogeneousTransform().toNumPy()
