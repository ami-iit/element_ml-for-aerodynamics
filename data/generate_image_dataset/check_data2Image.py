...
def main():
    ...
    joint_positions = joint_configs[joint_configs[:,0] == joint_config_name][0,1:].astype(float)*np.pi/180
    ...
    flow.import_raw_fluent_data(data_path, joint_config_name, pitch_angle, yaw_angle, robot.surface_list)        
    ...
    if SAVE_IMAGES:
        ...
        print("Saving images ...")
        np.save(str(image_dir/f"{joint_config_name}-{pitch_angle}-{yaw_angle}.npy"), flow.image)
    ...
    for surface_index, surface_name in enumerate(robot.surface_list):
        ...
        ax.set_ylabel(r'$z$ [m]')
        ax.set_xlim([np.min(flow.surface[surface_name].theta), np.max(flow.surface[surface_name].theta)])
        ax.set_ylim([np.min(flow.surface[surface_name].z), np.max(flow.surface[surface_name].z)])
    ...
    fig = plt.figure("Assembled Image")
    manager = plt.get_current_fig_manager()
    manager.window.showMaximized()
    ax3 = fig.add_subplot(1, 1, 1)
    image = ax3.imshow(flow.image[0,:,:], origin='upper', cmap='jet', vmax=1, vmin=-2)
    ax3.axis("off")
    fig.colorbar(image, ax=ax3, orientation='vertical', fraction=0.02, pad=0.45)
    plt.show(block=False)
