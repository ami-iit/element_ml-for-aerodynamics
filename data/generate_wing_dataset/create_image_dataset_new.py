...
def main():
    ...
    data_dir = root / "simulations" / "new_database"
    ...
    for idx, file in enumerate(files):
        sweep = float(file.parent.stem.split("_")[-1])
        aoa = float(file.stem[16:])
        wing_name = file.parent.stem
        # Import and set mesh mapping
        map_file = root / "maps" / f"{wing_name}-map-eem-{DENSITY_EXP}.npy"
